...
namespace AaiEduHr\HeartPhrameModuleComment\Command;
...
/**
 * HR: Kopira jedinu početnu Comment migraciju u host aplikaciju.
 * EN: Copies the single initial Comment migration into a host application.
 */
final readonly class HpCommentCommand
{
    private const TEMPLATE = 'resources/migrations/initial_comment_schema.php';
    ...
    /**
     * HR: Kopira početni predložak bez naknadnih parcijalnih migracija.
     * EN: Copies the initial template without follow-up partial migrations.
     *
     * @param array<int, string> $arguments
     * @param array<string, mixed> $options
     */
    public function installMigration(array $arguments = [], array $options = []): int
    {
        ...
        if (!is_file($template)) {
            throw new RuntimeException(__('Početna Comment migracija nije pronađena.'));
        }
        ...
        $target = rtrim($directory, DIRECTORY_SEPARATOR)
            . DIRECTORY_SEPARATOR
            . date('YmdHis')
            . '_install_comment_module_schema.php';
        $content = file_get_contents($template);
        if (!is_string($content) || file_put_contents($target, $content) === false) {
            throw new RuntimeException(__('Nije moguće kopirati početnu Comment migraciju.'));
        }
    ...
    {
        echo 'hph comment <install|help>' . PHP_EOL;
    ...
    {
        echo __('Nepoznata Comment podnaredba: ') . $subcommand . PHP_EOL;
